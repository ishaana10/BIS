...
function nuRunReportId($jsonID, $tag, $get) {
	...
	// ───────────────────────────────────────────────
// Custom report background (color + image)
// ───────────────────────────────────────────────

// Get values from nuREPORT (they should be available after nuGetJSONData + hash setup)
// Fallback: pull from nuREPORT if available (sometimes populated)
$bgColor = $GLOBALS['nuREPORT']['bgcolor'] ?? ($GLOBALS['nuHash']['bgcolor'] ?? '');
$bgImage = $GLOBALS['nuREPORT']['bgimage'] ?? ($GLOBALS['nuHash']['bgimage'] ?? '');

// Or force into hash
$GLOBALS['nuHash']['bgcolor'] = $bgColor;
$GLOBALS['nuHash']['bgimage'] = $bgImage;


// After getting $bgColor and $bgImage ...

if ($bgColor === '' || $bgColor === '#ffffff' || $bgColor === '#FFFFFF') {
    $bgColor = '#ffffff';  // explicit white → prevents black fallback
} else {
    // Validate hex roughly
    if (!preg_match('/^#?[0-9A-Fa-f]{6}$/i', $bgColor)) {
        $bgColor = '#ffffff';  // invalid → white
    }
}

$rgb = hex2rgb($bgColor);
$PDF->SetFillColor($rgb[0], $rgb[1], $rgb[2]);
$PDF->Rect(0, 0, $PDF->getPageWidth(), $PDF->getPageHeight(), 'F');  // always fill something safe


// ── Background IMAGE ───────────────────────────────────────
if ($bgImage !== '') {
    // nuBuilder files are stored in zzzzsys_file → get real path or use nuDownload.php
    // Most reliable: temporary file like images already do
    $imagePath = nuCreateFile($bgImage);   // reuses existing logic from nuIsImage / nuCreateFile

    if ($imagePath && file_exists($imagePath)) {
        $GLOBALS['nu_files'][] = $imagePath;   // ensure cleanup

        // Register background image to be drawn on every page
        // We'll draw it in a new function called from AddPage hook or per section

        // Option A: Draw once per AddPage (recommended)
        // We'll add a custom AddPage override below
    }
}
...
function nuPrintReport($PDF, $LAY, $DATA, $JSON, $isHTML){
	...
	for($s = 0 ; $s < $countDATA ; $s++){
		...
		if($lastSectionTop > $DATA[$s]->sectionTop){
			...
			//$bgColor = '#f0f8ff';
			//$bgImage = '/certbg.jpg';
//nuDrawPageBackground($PDF, $bgColor, $bgImage);
		}

// Draw page background (color + image)
//nuDrawPageBackground($PDF, $bgColor, $bgImage, $imagePath ?? '');
...
// In nuDrawPageBackground($PDF, $bgColor, $bgImage)
function nuDrawPageBackground($PDF, $bgColor = '', $bgImage = '') {
    $pageW = $PDF->getPageWidth();
    $pageH = $PDF->getPageHeight();

    // Color: default to white if empty/invalid
    if ($bgColor === '' || strtolower($bgColor) === '#ffffff') {
        $bgColor = '#ffffff';
    }
    $rgb = hex2rgb($bgColor);
    $PDF->SetFillColor($rgb[0], $rgb[1], $rgb[2]);
    $PDF->Rect(0, 0, $pageW, $pageH, 'F');   // always fill page

    // Image on top (if present)
    if ($bgImage !== '') {
        $imgPath = nuCreateFile($bgImage);   // this should work if file code exists
        if ($imgPath && file_exists($imgPath)) {
            $GLOBALS['nu_files'][] = $imgPath;
            $PDF->Image($imgPath, 0, 0, $pageW, $pageH, '', '', '', false, 300, '', false, false, 0, 'CM', false, false);
            nuDebug("Background image drawn from: $imgPath");
        } else {
            nuDebug("Failed to load bg image: $bgImage → path: " . ($imgPath ?: 'null'));
        }
    } else {
        nuDebug("No bg image specified");
    }

    nuDebug("Page background filled with color: $bgColor");
}
?>
