...
/*function nuReportDialog() {
	...
	top = nuDialogInput('set background', 'choosebg', top, 180, nuREPORT, fun, [['Y', 'Yes'], ['N', 'No']]);

// ───────────────────────────────────────────────
    // New: Background Image (uses nuBuilder file chooser)
    // ───────────────────────────────────────────────

/*$('#nuDragDialog').append(
        '<div style="position:absolute; left:20px; top:' + top + 'px; width:160px; text-align:right;">' +
        '<label for="bgimage">Background Image:</label></div>'
    );
    
    var currentImg = nuREPORT.bgimage || '';
    $('#nuDragDialog').append(
        '<input type="text" id="bgimage" readonly ' +
        'style="position:absolute; left:190px; top:' + top + 'px; width:140px;" ' +
        'value="' + currentImg + '">'
    );
    
    $('#nuDragDialog').append(
        '<input type="button" value="Choose..." ' +
        'style="position:absolute; left:340px; top:' + top + 'px;" ' +
        'onclick="nuPopupFile(\'bgimage\', \'bgimage_callback\')">'
    );
    
    top += 40;*/

    // Disable width/height as before
    //$('#width').attr('disabled', true);
   // $('#height').attr('disabled', true);

    // Adjust dialog height
    //$('#nuDragDialog').css('height', top + 30);
    //$('#nuDragDialog').css('visibility', 'visible');
   // $('#nuModal').remove();

    // Optional: small helper function called after file selection
   // window.bgimage_callback = function(fileName) {
       // $('#bgimage').val(fileName);
       // nuREPORT.bgimage = fileName;
        //nuUpdateReport()
	/*$('#width').attr('disabled', true);
	$('#height').attr('disabled', true);
...
function nuReportDialog() {
    nuDragD.createDialog(400, window.scrollY + 50, 450, 550, opener.nuTranslate('Report Properties'));
    ...
    top = nuDialogInput('Paper', 'paper', top, 180, nuREPORT, fun, 
        [['A4','A4'], ['A5','A5'], ['Letter','Letter'], ['Legal','Legal']]);
    
    top = nuDialogInput('Orientation', 'orientation', top, 180, nuREPORT, fun, 
        [['P','Portrait'], ['L','Landscape']]);
    
    top = nuDialogInput('Use HTML', 'ishtml', top, 180, nuREPORT, fun, 
        [['Y','Yes'], ['N','No']]);
    
    top = nuDialogInput('Set background', 'choosebg', top, 180, nuREPORT, fun, 
        [['Y','Yes'], ['N','No']]);

    // ───────────────────────────────────────────────
    // New: Background Color (color picker)
    // ───────────────────────────────────────────────
    top += 10;  // small spacing
    $('#nuDragDialog').append(
        '<div style="position:absolute; left:20px; top:' + top + 'px; width:160px; text-align:right;">' +
        '<label for="bgcolor">Background Color:</label></div>'
    );
    $('#nuDragDialog').append(
        '<input type="color" id="bgcolor" ' +
        'style="position:absolute; left:190px; top:' + (top-4) + 'px; width:140px; height:28px;" ' +
        'onchange="nuREPORT.bgcolor = this.value; nuUpdateReport();" ' +
        'value="' + (nuREPORT.bgcolor || '#ffffff') + '">'
    );
    top += 40;
